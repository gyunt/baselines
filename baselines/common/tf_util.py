...
def load_variables(load_path, variables=None, sess=None, different_size=True):
    import joblib
    sess = sess or get_session()
    variables = variables or tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
    ...
    if isinstance(loaded_params, list):
        ...
        for v in variables:
            try:
                if different_size and not np.all(v.get_shape() == loaded_params[v.name].shape):
                    warnings.warn('shape of {} is different'.format(v.name))
                    indices = np.minimum(v.get_shape().as_list(), loaded_params[v.name].shape)
                    slices = tf.strided_slice(v, np.zeros(indices.shape, dtype=np.int), indices, var=v, name='v_slice')

                    params = loaded_params[v.name]
                    for i in indices:
                        params = params[:i]
                    restores.append(tf.assign(slices, params))
                else:
                    restores.append(v.assign(loaded_params[v.name]))
            except KeyError:
                warnings.warn("{} is missing in model file.".format(v.name))
    sess.run(restores)
