...
class PolicyWithTwoValueHeads(object):
    """
    ...
    def __init__(self, env, observations, latent, estimate_q=False, vf_latent=None, sess=None, states=None, prev=None,
        ...
        **tensors       tensorflow tensors for additional attributes such as state or mask
        ...
        if estimate_q:
            ...
            self.vf_int = fc(vf_latent, 'vf_int', 1, init_scale=0.01)
            self.vf_int = self.vf_int[:, 0]
            self.vf_ext = fc(vf_latent, 'vf_ext', 1, init_scale=0.01)
            self.vf_ext = self.vf_ext[:, 0]

        self.step_output = {
            'actions': self.action,
            'int_values': self.vf_int,
            'ext_values': self.vf_ext,
    ...
    def values(self, **kwargs):
        return self.sess.run({'int_values': self.vf_int,
                              'ext_values': self.vf_ext
                              },
                             feed_dict=self.feed_dict(**kwargs))
...
def build_rnd_ppo_policy(env, policy_network, value_network=None, normalize_observations=False, estimate_q=False,
                         **policy_kwargs):
    ...
    def policy_fn(nbatch=None, nsteps=None, sess=None, observ_placeholder=None):
        ...
        with tf.variable_scope('pi', reuse=tf.AUTO_REUSE):
            if 'nenv' in inspect.getfullargspec(policy_network).args:
                nenv = nbatch // nsteps
        ...
        if value_network_ is None or value_network_ == 'shared':
            ...
            with tf.variable_scope('vf', reuse=tf.AUTO_REUSE):
                if 'nenv' in inspect.getfullargspec(value_network_).args:
                    nenv = nbatch // nsteps
        ...
        policy = PolicyWithTwoValueHeads(
            env=env,
            observations=X,
